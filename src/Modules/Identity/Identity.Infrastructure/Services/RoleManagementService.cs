...
public sealed class RoleManagementService(BusinessDbContext businessDbContext) : IRoleManagementService
{
    ...
    public async Task UnassignRoleAsync(int userId, int roleId, CancellationToken cancellationToken)
    {
        ...
        if (userRole is null)
        {
            throw new NotFoundAppException($"Kullanıcı-role ataması bulunamadı. userId={userId}, roleId={roleId}");
        }
    ...
    public async Task DeleteRoleAsync(int roleId, CancellationToken cancellationToken)
    {
        ...
        {
            throw new ForbiddenAppException("Sistem role kayıtları silinemez.");
        }
        ...
        {
            throw new ValidationAppException(
                "Role silme doğrulaması başarısız.",
                new Dictionary<string, string[]>
                {
                    ["roleId"] = ["Role aktif kullanıcı ataması içerdiği için silinemez. Önce atamaları kaldırın."]
                });
        }
