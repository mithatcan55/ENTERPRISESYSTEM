...
public sealed class UserPermissionService(BusinessDbContext businessDbContext) : IUserPermissionService
{
    ...
    public async Task DeleteActionPermissionAsync(int permissionId, CancellationToken cancellationToken)
    {
        if (permissionId <= 0)
        {
            throw new ValidationAppException(
                "Permission silme doğrulaması başarısız.",
                new Dictionary<string, string[]>
                {
                    ["permissionId"] = ["Geçerli bir permissionId zorunludur."]
                });
        }
        ...
        if (permission is null)
        {
            throw new NotFoundAppException(
                $"Permission bulunamadı. permissionId={permissionId}",
